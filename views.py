...
def exam(request):
    ...
    # Reset tab switch count for new exam - always start from 0
    # Delete any previous tab switch events to ensure a fresh start
    CheatingEvent.objects.filter(student=student, event_type='tab_switch').delete()
...
def result(request):
    ...
    # Calculate cheating severity score (0-100)
    cheating_severity = 0
    cheating_details = []
    ...
    for event in cheating_events:
        ...
        if event.cheating_flag:
            ...
            if event.detected_objects:
                ...
                reason += f"Detected Objects: {', '.join(event.detected_objects)}. "
            
            # Event type severity
            if event.event_type == 'audio_detection':
                severity += 25
                reason += "External Audio Detected. "
            elif event.event_type == 'gaze_tracking':
                severity += 15
